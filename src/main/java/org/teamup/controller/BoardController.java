...
private static final Logger logger = LoggerFactory.getLogger(BoardController.class);
	...
	public void	designList(Model model, @ModelAttribute("cri") Criteria cri) throws Exception{
		cri.setCategoryId(3);
	...
	public void homeList(Model model) throws Exception{
		logger.info("Welcome homeList.");
		
	}
	
	@RequestMapping(value = "/itList", method = RequestMethod.GET)
	public void itList(Model model, @ModelAttribute("cri") Criteria cri) throws Exception{
        cri.setCategoryId(1);
        model.addAttribute("list", service.listCriteria(cri));        
    ...
    public void readWrite(Model model, @RequestParam("boardId") Integer boardId, HttpSession session) throws Exception {
        ...
        model.addAttribute("board",service.readBoard(boardId));
	...
	public String updateWritePOST(BoardVO vo, HttpSession session) throws Exception{
		logger.info("Welcome updateWrite.");
		service.updateBoard(vo);
		return "redirect:/board/marketingList";
	}
    ...
    public String participate(@RequestParam("boardId") int boardId, HttpSession session ) throws Exception{
        logger.info("Welcome updateWrite.");
        PrtcMember pm = new PrtcMember();
        pm.setMemberId(((MemberVO)session.getAttribute("member")).getMemberId());
        pm.setBoardId(boardId);
    ...
    public String participate_out(@RequestParam("boardId") int boardId, HttpSession session) throws Exception{
        logger.info("Welcome updateWrite.");
        PrtcMember pm = new PrtcMember();
        pm.setMemberId(((MemberVO)session.getAttribute("member")).getMemberId());
        pm.setBoardId(boardId);
